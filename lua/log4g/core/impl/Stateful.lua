...
local function _getNewInstanceIndex(prevIndex)
    if type(prevIndex) == "function" then return function(instance, name) return _getStatefulMethod(instance, name) or prevIndex(instance, name) end end
...
local function _assertType(val, name, expected_type, type_to_s)
    if type(val) ~= expected_type then
        error("Expected " .. name .. " to be of type " .. (type_to_s or expected_type) .. ". Was " .. tostring(val) .. "(" .. type(val) .. ")")
    end
end
