...
local function initLevel()
	...
	function Level:IsInRange(l1, l2)
		...
		if
			(self:IntLevel() >= l1:IntLevel() and self:IntLevel() <= l2:IntLevel())
			or (self:IntLevel() <= l1:IntLevel() and self:IntLevel() >= l2:IntLevel())
		then
			return true
		end
...
local function initLogger()
	...
	function LoggerConfig:SetParent(T)
		if type(T) == "string" then
			if not hasLoggerConfig(T, ContextDict[self:GetContext()]) then
				return
			end
